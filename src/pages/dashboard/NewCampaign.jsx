...
export default function NewCampaign() {
  ...
  const [campaign, setCampaign] = useState({ name: "Summer Drop '25", startDate: '2025-06-01', endDate: '2025-08-31', status: 'Draft' })
  const [socialLinks, setSocialLinks] = useState({ instagram: '', youtube: '', spotify: '', tiktok: '', twitter: '' })

  const addBlock = (type) => {
    const defaults = { Header: 'New Header', Text: 'Add your text here...', Button: 'Click Me', Image: null, Video: null, Audio: null, Footer: '© 2025 · Powered by InteliMerch' }
    setBlocks(b => [...b, { id: Date.now(), type, content: defaults[type] || '', meta: {} }])
  }
  const removeBlock = (id) => setBlocks(b => b.filter(x => x.id !== id))
  const updateBlock = (id, field, value) => {
    if (field === 'content') setBlocks(b => b.map(x => x.id === id ? { ...x, content: value } : x))
    else setBlocks(b => b.map(x => x.id === id ? { ...x, meta: { ...x.meta, [field]: value } } : x))
  }
  ...
  const onDragOver = (e, id) => { e.preventDefault(); if (id !== draggingId) setDragOverId(id) }
          ...
          <div className="space-y-3">
            <div>
              ...
              <input value={campaign.name} onChange={e => setCampaign(c => ({ ...c, name: e.target.value }))}
                className="input-field text-sm py-2" placeholder="Campaign name..." />
            </div>
            <div className="grid grid-cols-2 gap-2">
              <div>
                <label className="block text-xs font-semibold text-gray-700 mb-1">Start</label>
                <input type="date" value={campaign.startDate} onChange={e => setCampaign(c => ({ ...c, startDate: e.target.value }))} className="input-field text-xs py-2" />
              </div>
              <div>
                <label className="block text-xs font-semibold text-gray-700 mb-1">End</label>
                <input type="date" value={campaign.endDate} onChange={e => setCampaign(c => ({ ...c, endDate: e.target.value }))} className="input-field text-xs py-2" />
              </div>
            </div>
            <div>
              <label className="block text-xs font-semibold text-gray-700 mb-1">Status</label>
              <select value={campaign.status} onChange={e => setCampaign(c => ({ ...c, status: e.target.value }))} className="input-field text-sm py-2">
        ...
        <div className="p-4 border-b border-gray-100">
          ...
          <div className="relative">
            <button onClick={() => setShowProductDrop(!showProductDrop)}
              className="input-field text-left flex items-center justify-between text-sm py-2">
            ...
            {showProductDrop && (
              <div className="absolute top-full left-0 right-0 bg-white border border-gray-200 rounded-xl shadow-lg z-20 py-1 mt-1">
                {productList.map(p => (
                  <button key={p} onClick={() => toggleProduct(p)} className="flex items-center gap-2 w-full px-3 py-2.5 text-sm hover:bg-gray-50">
          ...
          <div className="space-y-2.5">
            {socialPlatforms.map(({ key, label, placeholder, color }) => (
              <div key={key}>
                ...
                <input value={socialLinks[key]} onChange={e => setSocialLinks(s => ({ ...s, [key]: e.target.value }))}
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        <div className="bg-white border-b border-gray-100 px-6 py-3.5 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            <button className="btn-outline w-auto px-4 py-2 text-sm gap-2"><Save size={14} /> Save Draft</button>
            <button className="btn-primary w-auto px-4 py-2 text-sm gap-2"><Zap size={14} /> Publish</button>
            <button onClick={() => navigate('/dashboard/campaigns')} className="btn-outline w-auto px-4 py-2 text-sm"><ArrowLeft size={14} /> Back</button>
          </div>
        </div>
                ...
                <div key={block.id} draggable
                  ...
                  <div className="flex items-center justify-between px-4 py-2 bg-gray-50/60 rounded-t-xl border-b border-gray-100">
                    ...
                    <div className="flex items-center gap-1">
                      ...
                      <button onClick={() => removeBlock(block.id)} className="p-1 hover:bg-red-50 rounded-lg">
  ...
  return (
    <div className="space-y-2.5" onClick={e => e.stopPropagation()}>
      {(block.type === 'Header' || block.type === 'Footer') && (
        <input autoFocus value={block.content || ''} onChange={e => onChange(block.id, 'content', e.target.value)}
          className="input-field text-sm" placeholder={`${block.type} text...`} />
      )}
      {block.type === 'Text' && (
        <textarea autoFocus value={block.content || ''} onChange={e => onChange(block.id, 'content', e.target.value)}
          className="input-field text-sm resize-none h-20" placeholder="Body text..." />
      )}
      {block.type === 'Button' && (
        <>
          <input autoFocus value={block.content || ''} onChange={e => onChange(block.id, 'content', e.target.value)}
            className="input-field text-sm" placeholder="Button label..." />
          <input value={block.meta?.url || ''} onChange={e => onChange(block.id, 'url', e.target.value)}
            className="input-field text-sm" placeholder="Link URL (optional)..." />
        </>
      )}
      {block.type === 'Video' && (
        <input autoFocus value={block.meta?.url || ''} onChange={e => onChange(block.id, 'url', e.target.value)}
          className="input-field text-sm" placeholder="Video embed URL (YouTube, Vimeo...)..." />
      )}
      {block.type === 'Audio' && (
        <>
          <input autoFocus value={block.content || ''} onChange={e => onChange(block.id, 'content', e.target.value)}
            className="input-field text-sm" placeholder="Track title..." />
          <input value={block.meta?.url || ''} onChange={e => onChange(block.id, 'url', e.target.value)}
            className="input-field text-sm" placeholder="Audio file URL..." />
        </>
      )}
      {block.type === 'Image' && (
        <>
          <input autoFocus value={block.meta?.url || ''} onChange={e => onChange(block.id, 'url', e.target.value)}
            className="input-field text-sm" placeholder="Image URL..." />
          <input value={block.content || ''} onChange={e => onChange(block.id, 'content', e.target.value)}
            className="input-field text-sm" placeholder="Caption / alt text..." />
        </>
      )}
    </div>
  )
